        ...
        async function loadDashboardData() {
            try {
                console.log('🔄 [loadDashboardData] Loading dashboard statistics...');
                ...
                if (upcomingError) {
                    console.error('Error loading upcoming bookings count:', upcomingError);
                }
                ...
                if (todayError) {
                    console.error('Error loading today bookings count:', todayError);
                }
        ...
        async function createAdminAccount() {
            ...
            alert(
                ...
                '• Provide a full name for the admin\n\n' +
        ...
        async function bootstrapData(user) {
            try {
                ...
                // Create user profile if it still doesn't exist after retries
                if (!userProfile) {
                    console.log('⚠️ No user profile found after retries, creating one...');
                    try {
                        const { data: newUserProfile, error: userCreateError } = await supabaseClient
                            .from('users')
                            .insert({
                                id: user.id,
                                email: user.email,
                        ...
                        if (userCreateError) {
                            console.error('❌ Could not create user profile:', userCreateError);
                ...
                while (!startup && retryCount < maxRetries) {
                    ...
                    if (error && error.code !== 'PGRST116') {
                        console.error(`❌ Error loading startup profile (attempt ${retryCount + 1}):`, error);
                        retryCount++;
                        if (retryCount < maxRetries) {
                            console.log('⏳ Retrying in 1 second...');
                            await new Promise(resolve => setTimeout(resolve, 1000));
                ...
                if (startup) {
                    console.log('✅ Startup profile found:', startup);
                    currentStartup = startup;
                } else {
                    console.log('⚠️ No startup profile found');
                    currentStartup = null;
                    ...
                    if (user.user_metadata?.startup_name && currentUser.role === 'startup') {
                        console.log('🔧 Attempting to create startup profile from metadata...');
                        try {
                            ...
                            if (startupCreateError) {
                                console.error('❌ Could not create startup profile:', startupCreateError);
                            } else {
                                console.log('✅ Startup profile created from metadata:', newStartup);
                                currentStartup = newStartup;
                            }
                        } catch (createError) {
                            console.error('❌ Exception creating startup profile:', createError);
                        }
                    } else if (currentUser.role === 'admin') {
                        // Auto-provision a "NIC ADMIN" startup for admin users to enable bookings
                        console.log('🛠️ Auto-provisioning NIC ADMIN startup for admin user...');
                        try {
                            ...
                            if (adminStartupError) {
                                console.error('❌ Failed to create NIC ADMIN startup:', adminStartupError);
                                ...
                                const { data: fallbackStartup } = await supabaseClient
                                    ...
                                    .maybeSingle();
                                if (fallbackStartup) {
                                    console.log('✅ Found existing startup after conflict:', fallbackStartup);
                                    currentStartup = fallbackStartup;
                                }
                            } else if (adminStartup) {
                                console.log('✅ NIC ADMIN startup created:', adminStartup);
                                currentStartup = adminStartup;
                            }
                        } catch (adminCreateErr) {
                            console.error('❌ Exception creating NIC ADMIN startup:', adminCreateErr);
                        }
                    }
                }
        ...
        function updateUIForUserRole(role) {
            ...
            if (currentUser) {
                ...
                if (headerUserNameEl) headerUserNameEl.textContent = currentUser.name || 'User';
        ...
        signupFormEl?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (authData.session && authData.user) {
                    console.log('📝 Step 2: Creating user profile (session present)...');
                    const { error: userError } = await supabaseClient
                        .from('users')
                        .insert({
                            id: authData.user.id,
        ...
        loginFormEl?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                loginErrorEl.textContent = err.message || 'Login failed. Please try again.';
            }
        });
        ...
        */
        ...
        bookingFormEl?.addEventListener('submit', async (e) => {
            ...
            if (!currentStartup && currentUser) {
                console.log('🔄 Lazy-loading startup profile for booking...');
                try {
                    ...
                    if (error && error.code !== 'PGRST116') {
                        console.error('❌ Error lazy-loading startup:', error);
                    } else if (startup) {
                        console.log('✅ Startup profile lazy-loaded:', startup);
                        currentStartup = startup;
                        updateProfileDebugInfo?.();
                    } else {
                        console.log('⚠️ No startup profile found during lazy-load');
                    }
                } catch (err) {
                    console.error('❌ Exception during startup lazy-load:', err);
                }
            }
            ...
            if (!currentStartup) {
                if (currentUser?.role === 'admin') {
                    ...
                    showBookingBanner('No startup profile found. Go to Settings → Create Missing Startup Profile to fix this issue.', 'red');
                    return;
                }
            }
        ...
        function renderAdminBookingsList(bookings, startupMap) {
            ...
            list.innerHTML = bookings.map(b => {
                ...
                const confidentialBadge = b.is_confidential ? '<span class="ml-2 px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded">🔒 Confidential</span>' : '';
                            ...
                            <div class="flex-1">
                                ...
                                <div class="grid grid-cols-2 gap-2 text-sm text-gray-700 mb-2">
                                    ...
                                    <div>
                                        ...
                                        ${b.start_time} • ${b.duration}h duration
                                    </div>
                                </div>
                                <div class="text-xs text-gray-600">
                                    ...
                                    <strong>${startupName}</strong>${contact ? ' • ' + contact : ''}
                                </div>
        ...
        window.adminCancelBooking = async function(bookingId) {
            try {
                ...
                console.log('🔄 [adminCancelBooking] Booking ID:', bookingId);
                ...
                if (error) {
                    console.error('❌ [adminCancelBooking] Database error:', error);
                ...
                if (!data || data.length === 0) {
                    console.warn('⚠️ [adminCancelBooking] No rows deleted - booking may not exist or RLS policy blocking');
                ...
                console.log('🔄 [adminCancelBooking] Reloading booking lists...');
                await loadAdminBookingsData();
                await loadMyBookingsData();

                console.log('✅ [adminCancelBooking] Complete!');
            } catch (e) {
        ...
        window.adminEditBooking = async function(bookingId) {
            try {
                console.log('🔧 [adminEditBooking] Opening edit modal for booking:', bookingId);

                if (currentUser?.role !== 'admin') {
                    console.error('❌ [adminEditBooking] User is not admin');
                ...
                console.log('✅ [adminEditBooking] Loaded booking:', booking);
                ...
                console.error('❌ [adminEditBooking] Error:', error);
        ...
        async function saveEditedBooking(e) {
            ...
            console.log('💾 [saveEditedBooking] Saving changes for booking:', bookingId);
            ...
            try {
                ...
                console.log('✅ [saveEditedBooking] Booking updated successfully');
                ...
                console.log('🔄 [saveEditedBooking] Refreshing booking lists...');
                await loadAdminBookingsData();
                await loadMyBookingsData();

            } catch (error) {
                console.error('❌ [saveEditedBooking] Error:', error);
        ...
        window.adminDeleteBooking = async function(bookingId) {
            try {
                console.log('🗑️ [adminDeleteBooking] Starting permanent deletion for booking:', bookingId);

                if (currentUser?.role !== 'admin') {
                    console.error('❌ [adminDeleteBooking] User is not admin');
                ...
                if (confirmText !== 'DELETE') {
                    console.log('⚠️ [adminDeleteBooking] Deletion cancelled - confirmation failed');
                    showNotification('Deletion cancelled', 'info');
                    return;
                }

                console.log('🔄 [adminDeleteBooking] Permanently deleting booking from database...');
                console.log('🔄 [adminDeleteBooking] Booking ID:', bookingId);
                ...
                if (error) {
                    console.error('❌ [adminDeleteBooking] Database error:', error);
                    throw error;
                }

                if (!data || data.length === 0) {
                    console.warn('⚠️ [adminDeleteBooking] No rows deleted - booking may not exist');
                    showNotification('Deletion failed - booking not found', 'error');
                    return;
                }

                console.log('✅ [adminDeleteBooking] Booking permanently deleted:', data);
                showNotification('Booking permanently deleted', 'success');

                console.log('🔄 [adminDeleteBooking] Reloading booking lists...');
                await loadAdminBookingsData();
                await loadMyBookingsData();

                console.log('✅ [adminDeleteBooking] Complete!');
            } catch (e) {
                console.error('❌ [adminDeleteBooking] Exception:', e);
        ...
        function filterAdminBookings() {
            ...
            console.log('🔍 [filterAdminBookings] Filtering with:', { searchText, filterRoom, filterStatus });
            ...
            console.log(`✅ [filterAdminBookings] Showing ${filtered.length} of ${allAdminBookings.length} bookings`);
        ...
        document.getElementById('edit-booking-modal-close')?.addEventListener('click', () => {
            console.log('🔧 [Edit Booking Modal] Closing modal (X button)');
            document.getElementById('edit-booking-modal').classList.add('hidden');
        });

        document.getElementById('edit-booking-cancel-btn')?.addEventListener('click', () => {
            console.log('🔧 [Edit Booking Modal] Closing modal (Cancel button)');
            document.getElementById('edit-booking-modal').classList.add('hidden');
        });
        ...
        document.getElementById('cancel-booking-modal-close')?.addEventListener('click', () => {
            console.log('🔧 [Cancel Booking Modal] Closing modal (X button)');
        ...
        document.getElementById('cancel-booking-cancel-btn')?.addEventListener('click', () => {
            console.log('🔧 [Cancel Booking Modal] Closing modal (Keep Booking button)');
            document.getElementById('cancel-booking-modal').classList.add('hidden');
        });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            createStartupBtn?.addEventListener('click', async () => {
                ...
                if (statusEl) {
                    statusEl.innerHTML = '<span class="text-blue-600">🔄 Creating startup profile...</span>';
                }

                try {
                    console.log('🔧 Creating startup profile manually...');
                    console.log('🔧 User ID:', currentUser.id);
                    console.log('🔧 Startup Name:', startupName.trim());
                    ...
                    if (error) {
                        ...
                        if (statusEl) {
                            statusEl.innerHTML = '<span class="text-red-600">❌ Failed to create startup profile</span>';
                        }

                        alert(errorMessage);
                    } else {
                        console.log('✅ Startup profile created successfully:', data);
                        currentStartup = data;
                        ...
                        if (statusEl) {
                            statusEl.innerHTML = '<span class="text-green-600">✅ Startup profile created successfully! You can now book rooms.</span>';
                        }

                        alert('✅ Startup profile created successfully! You can now book rooms.');
                    }
                } catch (err) {
                    console.error('❌ Exception creating startup profile:', err);

                    if (statusEl) {
                        statusEl.innerHTML = '<span class="text-red-600">❌ Error creating startup profile</span>';
                    }
            ...
            reloadBtn?.addEventListener('click', async () => {
                console.log('🔄 Reloading all profile data...');
                if (currentUser) {
                    await bootstrapData({
                        id: currentUser.id,
                        email: currentUser.email,
                    ...
                    const statusEl = document.getElementById('profile-actions-status');
                    if (statusEl) {
                        statusEl.innerHTML = '<span class="text-blue-600">🔄 Profile data reloaded</span>';
                    }
                }
            });
        ...
        function renderCalendar(date) {
            ...
            for (let i = 0; i < 42; i++) {
                ...
                if (selectedDate && currentDate.toISOString().split('T')[0] === selectedDate) {
                    if (currentDate.toDateString() !== today.toDateString()) {
                ...
                bookings.forEach(booking => {
                    ...
                    scheduleHTML += `
                        <div class="border-2 rounded-xl p-6 hover:shadow-md transition-all duration-200 ${statusBg} ${statusBorder}">
                            ...
                            <!-- Admin Cancel Button -->
                            ${currentUser && currentUser.role === 'admin' && status !== 'Cancelled' ? `
                                <div class="pt-4 border-t border-gray-200">
                                    <button onclick='showCancelBookingModal("${booking.id}")' class="w-full md:w-auto px-6 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-200 font-medium shadow-sm hover:shadow-md flex items-center justify-center space-x-2">
        ...
        async function showCancelBookingModal(bookingId) {
            console.log('📅 [Schedule] Opening cancel booking modal for:', bookingId);
        ...
        async function handleCancelBooking(e) {
            ...
            console.log('📅 [Schedule] Cancelling booking:', bookingId, 'Reason:', reason);
            ...
            try {
                ...
                console.log('✅ [Schedule] Booking cancelled successfully');
                showNotification('Booking cancelled successfully', 'success');
        ...
        async function handleCreateAdmin(e) {
            ...
            try {
                ...
                console.log('📝 [handleCreateAdmin] Name:', name);
                ...
                if (authError) {
                    console.error('❌ [handleCreateAdmin] Auth error:', authError);
                    throw authError;
                }
                ...
                if (userError) {
                    console.error('❌ [handleCreateAdmin] User profile error:', userError);
                    console.error('❌ [handleCreateAdmin] Error details:', {
        ...
        async function loadContactData() {
            ...
            console.log('✅ Contact Us tab loaded (static content)');
            return;
        }
        ...
        async function editStartupUser(startupId) {
            try {
                console.log(`🔄 [editStartupUser] Loading startup ${startupId}...`);
                ...
                if (error) {
                    console.error('❌ [editStartupUser] Error fetching startup:', error);
                    showMessage('Failed to load startup data: ' + error.message, 'error');
                    return;
                }

                console.log('✅ [editStartupUser] Startup loaded:', startup);
                ...
                console.error('❌ [editStartupUser] Exception:', err);
        ...
        async function saveStartupUserChanges(e) {
            ...
            try {
                ...
                console.log(`🔄 [saveStartupUserChanges] Updating startup ${startupId}...`);
                ...
                if (error) {
                    console.error('❌ [saveStartupUserChanges] Database error:', error);
                ...
                console.log('✅ [saveStartupUserChanges] Startup updated successfully');
                ...
                // Reload contact data
                console.log('🔄 [saveStartupUserChanges] Reloading contact data...');
                await loadContactData();

            } catch (err) {
                console.error('❌ [saveStartupUserChanges] Exception:', err);
        ...
        async function deleteStartupUser(startupId, startupName) {
            try {
                console.log(`🔄 [deleteStartupUser] Deleting startup ${startupId}...`);
                ...
                if (!confirmed) {
                    console.log('⚠️ [deleteStartupUser] User cancelled deletion');
                    return;
                }
                ...
                if (typedName !== startupName) {
                    ...
                    console.log('⚠️ [deleteStartupUser] Name confirmation failed');
                    return;
                }

                console.log('🔄 [deleteStartupUser] Deleting from database...');
                ...
                if (error) {
                    console.error('❌ [deleteStartupUser] Database error:', error);
                ...
                console.log('✅ [deleteStartupUser] Startup deleted successfully');
                showMessage(`Startup "${startupName}" deleted successfully`, 'success');

                // Reload contact data
                console.log('🔄 [deleteStartupUser] Reloading contact data...');
                await loadContactData();

            } catch (err) {
                console.error('❌ [deleteStartupUser] Exception:', err);
        ...
        console.log('✅ Startup user management functions loaded');
        ...
        async function uploadFileToSupabase(file, contentType) {
            ...
            console.log('📤 [uploadFileToSupabase] Bucket: room-displays');
            console.log('📤 [uploadFileToSupabase] Path:', filePath);
            ...
            if (error) {
                console.error('❌ [uploadFileToSupabase] Upload failed:', error);
                throw error;
            }
        ...
        async function loadRoomStatusDisplays() {
            ...
            try {
                ...
                if (!allRoomsError && allRooms) {
                    ...
                    console.error('❌ [loadRoomStatusDisplays] Error fetching all rooms:', allRoomsError);
                }
                ...
                if (error) {
                    console.error('❌ [loadRoomStatusDisplays] Database error:', error);
                    throw error;
                }
                ...
                if (roomDisplays.length === 0) {
                    console.warn('⚠️ [loadRoomStatusDisplays] No room displays found in room_displays table');
                ...
                console.error('❌ [loadRoomStatusDisplays] Error:', error);
        ...
        async function saveDisplaySettings(e) {
            ...
            try {
                ...
                console.log('🔄 [saveDisplaySettings] Refreshing room displays...');
        ...
        document.getElementById('configure-modal-close')?.addEventListener('click', () => {
            console.log('🔧 [Configure Modal] Closing modal (X button)');
            document.getElementById('configure-display-modal').classList.add('hidden');
        });

        document.getElementById('configure-cancel-btn')?.addEventListener('click', () => {
            console.log('🔧 [Configure Modal] Closing modal (Cancel button)');
            document.getElementById('configure-display-modal').classList.add('hidden');
        });
        ...
        async function updateRoomStatus(roomId, newStatus) {
            try {
                ...
                console.error('❌ [updateRoomStatus] Exception:', err);
        ...
        async function syncRoomStatusWithBookings() {
            try {
                ...
                if (roomsError) {
                    console.error('❌ [syncRoomStatusWithBookings] Error fetching rooms:', roomsError);
                    return;
                }
                ...
                if (bookingsError) {
                    console.error('❌ [syncRoomStatusWithBookings] Error fetching bookings:', bookingsError);
                    return;
                }
                ...
                for (const room of rooms) {
                    ...
                    if (room.status === 'maintenance') {
                        console.log(`   ⚠️ Room is in MAINTENANCE - skipping auto-sync (admin override)`);
                        continue;
                    }
                    ...
                    for (const booking of roomBookings) {
                        ...
                        if (currentTime >= startTime && currentTime < endTime) {
                            ...
                            console.log(`   ✅ ACTIVE BOOKING FOUND! Room is OCCUPIED`);
                        ...
                        // Check if booking is upcoming (later today)
                            ...
                            console.log(`   📅 Upcoming booking found: ${startTime}`);
                        }
                    }
                    ...
                    if (!hasActiveBooking && hasUpcomingBooking) {
                        newStatus = 'reserved';
                        console.log(`   📅 Room is RESERVED (has upcoming booking)`);
                    } else if (!hasActiveBooking && !hasUpcomingBooking) {
                        console.log(`   ✅ Room is AVAILABLE (no bookings)`);
                    }

                    // Update room status if it changed
                    if (room.status !== newStatus) {
                        console.log(`   🔄 STATUS CHANGE DETECTED: ${room.status} → ${newStatus}`);
                        statusChanges++;
                        ...
                        if (updateError) {
                            console.error(`   ❌ Error updating rooms table:`, updateError);
                        } else {
                        ...
                        if (displayUpdateError) {
                            console.error(`   ❌ Error updating room_displays table:`, displayUpdateError);
                        } else {
                            console.log(`   ✅ Room_displays table updated successfully`);
                        }
                    } else {
                        console.log(`   ℹ️ No status change needed (already ${newStatus})`);
                    }
                }
                ...
                console.error('❌ [syncRoomStatusWithBookings] ========== EXCEPTION ==========');
        ...
        async function cancelCurrentBooking(roomName, bookingDate, startTime) {
            try {
                console.log(`🔄 [cancelCurrentBooking] Room: ${roomName}, Date: ${bookingDate}, Time: ${startTime}`);
                ...
                if (!confirmed) {
                    console.log('⚠️ [cancelCurrentBooking] User cancelled the action');
                    return;
                }

                console.log('🔄 [cancelCurrentBooking] Deleting booking from database...');
                ...
                if (error) {
                    console.error('❌ [cancelCurrentBooking] Database error:', error);
                    console.error('❌ [cancelCurrentBooking] Error details:', {
                        message: error.message,
                        details: error.details,
                        hint: error.hint,
                        code: error.code
                ...
                if (!data || data.length === 0) {
                    console.warn('⚠️ [cancelCurrentBooking] No rows deleted - booking may not exist or RLS policy blocking');
                    showMessage('Booking cancellation failed - no rows affected', 'error');
                    return;
                }

                console.log('✅ [cancelCurrentBooking] Booking deleted successfully:', data);
                showMessage('Booking cancelled successfully', 'success');

                // Sync room status (will set to available)
                console.log('🔄 [cancelCurrentBooking] Syncing room status...');
                await syncRoomStatusWithBookings();

                // Reload preview data
                console.log('🔄 [cancelCurrentBooking] Reloading preview...');
                ...
                console.log('✅ [cancelCurrentBooking] Complete!');

            } catch (err) {
                console.error('❌ [cancelCurrentBooking] Exception:', err);
        ...
        console.log('✅ window.cancelCurrentBooking is now available');
            ...
            return `
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition duration-200">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center space-x-2">
                            <span class="text-2xl">${typeIcons[content.content_type] || '📄'}</span>
                            <div>
                ...
                <div class="h-full flex flex-col ${statusColor.bg} text-white">
                    ...
                    <div class="p-4 sm:p-5 md:p-6 border-t ${statusColor.border} text-center">
                        <div class="text-base sm:text-lg md:text-xl lg:text-2xl opacity-75">
                            National Incubation Center • Room Display System
                        </div>
                    </div>
                </div>
            `;
